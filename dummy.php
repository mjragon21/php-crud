...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($unit) || empty($price) || empty($expiry_date) || empty($available_inventory) || empty($image_path)) {
        ...
        if ($_FILES["image_upload"]["error"] == UPLOAD_ERR_OK) {
            ...
            // Handle the case when the file was not uploaded successfully
            $errorMessage = "Error uploading the file: " . $_FILES["image_upload"]["error"];
        }
    } else {
        // Update the data in the database
        $sql = "UPDATE inventory SET name = '$name', unit = '$unit', price = '$price', expiry_date = '$expiry_date', available_inventory = '$available_inventory', image_path = '$image_path'   ERE id = $id";
        ...
        if (!$result) {
            ...
            // TODO: Handle file upload (implement this part)
            // ...
...
// Fetch data if it's not a POST request or if there is an error
